...
3.3.Treating NA’s and Negatives
```{r}
sum(is.na(data))
```


3.4.Uniariate Analysis  
```{r}
...
Therefore, every extra avergae roaming minute used by the customer increases the odds of Churn by 10.3% and every unit increase in the average number of Daytime minutes used by the customer the odds of Churn increases by 1.2%.
...
```
Therefore, we get a model with an area under the ROC curve of 79.7%. This indicates that there is an 79.7% chance that the model will correctly identify a customer who will retain or cancel their services with the telecom company.
...
```
Using K-Nearest Neighbours method we get an area under the curve value of 80.14% which is slightly better than the Logistic Regression model.
